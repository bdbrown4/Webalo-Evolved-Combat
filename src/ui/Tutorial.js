...
export class Tutorial {
  ...
  _pick(desktop, touch) { return this.game.isTouch ? touch : desktop; }
  ...
  _enterStep() {
    ...
    this.el.querySelector('.tut-exit').textContent = this._pick(`Press ${this._lbl('pause')} to exit training`, 'Tap ❚❚ to exit training');
    this.el.classList.toggle('info', step.kind === 'info');
  }

  _contHint(g) { return this._pick(`Press ${this._lbl('jump')} to continue`, 'Tap JUMP to continue'); }
  ...
  _buildSteps() {
    const T = this;
    const pick = (d, t) => T._pick(d, t);
    return [
      {
        kind: 'info', title: () => 'Welcome, Sergeant',
        body: (g) => `You're Sgt. Vance Orion — the last marine standing against the Wobble Coalition. This quick drill covers every control. ${pick('You\'ll use the <b>mouse + keyboard</b>.', 'You\'ll use the <b>on-screen stick and buttons</b>.')}`,
        check: (g) => g.input.pressed('jump'),
      },
      {
        kind: 'task', title: () => 'Look around',
        body: () => pick('Move the <b>mouse</b> to look around.', 'Drag anywhere on the <b>right half</b> of the screen to look around.'),
        check: () => T._lookAcc > 1.0,
      },
      {
        kind: 'task', title: () => 'Move',
        body: () => pick(`Walk with <b>${T._lbl('forward')} ${T._lbl('left')} ${T._lbl('back')} ${T._lbl('right')}</b>.`, 'Use the <b>left stick</b> to walk.'),
        check: (g) => g.player.pos.distanceTo(T._moveBase) > 3,
      },
      {
        kind: 'task', title: () => 'Sprint',
        body: () => pick(`Hold <b>${T._lbl('sprint')}</b> while moving forward to sprint.`, 'Push the <b>left stick</b> all the way forward to sprint.'),
        check: (g) => g.input.isDown('sprint') && g.input.isDown('forward'),
      },
      {
        kind: 'task', title: () => 'Jump',
        body: () => pick(`Press <b>${T._lbl('jump')}</b> to jump.`, 'Tap the <b>JUMP</b> button.'),
        check: (g) => g.input.pressed('jump'),
      },
      {
        kind: 'task', title: () => 'Open fire',
        body: () => pick('Aim with the mouse and <b>left-click</b> to shoot. Destroy the three targets.', 'Drag to aim, then <b>tap</b> the right side or hold <b>FIRE</b>. Destroy the three targets — aim-assist helps you connect.'),
      ...
      },
      {
        ...
        body: (g) => pick(`Press <b>${T._lbl('reload')}</b> to reload your magazine.`, 'Tap <b>RELOAD</b>, or <b>double-tap</b> the right side. (Empty mags also reload on their own.)'),
        check: (g) => g.input.pressed('reload') || (g.player.weapon && g.player.weapon.reloading > 0),
      },
      {
        kind: 'task', title: () => 'Swap weapons',
        body: () => pick(`You carry two weapons. Press <b>${T._lbl('swap')}</b> to switch between them.`, 'You carry two weapons. Tap <b>SWAP</b> to switch between them.'),
        check: (g) => g.player.current !== T._enterCurrent,
      },
      {
        kind: 'task', title: () => 'Throw a grenade',
        body: () => pick(`Press <b>${T._lbl('grenade')}</b> to cook and lob a grenade.`, 'Tap <b>NADE</b> — or press a <b>second finger</b> on the right side — to lob a grenade.'),
        check: (g) => T._totalNades(g.player) < T._enterNades,
      },
      {
        kind: 'task', title: () => 'Melee',
        body: () => pick(`Press <b>${T._lbl('melee')}</b> for a close-range melee strike.`, 'Tap <b>MELEE</b> for a close-range strike.'),
      ...
      },
      {
        ...
        body: () => pick(`Walk up to the glowing console and press <b>${T._lbl('interact')}</b> to use objects like consoles and doors.`, 'Walk up to the glowing console and tap <b>USE</b> to use objects like consoles and doors.'),
      ...
      },
      {
        ...
        cont: (g) => pick(`Press ${T._lbl('jump')} to return to the menu`, 'Tap JUMP to return to the menu'),
        check: (g) => g.input.pressed('jump'),
      },
    ];
  }
